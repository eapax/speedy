...
subroutine timint(j1,dt,eps,wil,nlev,field,fdt)
    ...
    type(rpe_complex_var) :: fieldOG(mx,nx,nlev,2)
    type(rpe_var) :: eps2
    ...
    fieldOG = field !make a copy of initial state

    ! Integration. Corrected version of Williams filter
    do k=1,nlev
        do n=1,nx
            do m=1,mx
                ...
                field(m,n,k,2) = fnew(m,n) -(rpe_literal(1.0_dp)-wil)*eps*(fieldOG(m,n,k,1)&
                   &-rpe_literal(2.0_dp)*field(m,n,k,j1)+fnew(m,n)) 
