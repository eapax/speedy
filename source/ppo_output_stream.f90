module ppo_output_stream
    ...
    contains
        ...
        subroutine update_output_stream(stream, istep)
            ...
            if (xmod(istep, stream%nstpopen)) then
               if (istep > 0) call close_output()
               call init_nc(stream)
            end if
        ...
        subroutine init_nc(stream)
            ...
            integer :: dimids(4)
            integer :: n
            character(len=100) :: lfilename
            character(len=8) :: fmt,flenum
            fmt = '(I5.5)'

            stream%filenum = stream%filenum + 1
            write (flenum,fmt) stream%filenum
            stream%rec = 1
            lfilename = trim(stream%filename)//trim(flenum)//'.nc'

            ! Open the netcdf file, assigning the file_ID
            call check( nf90_create(trim(lfilename), NF90_CLOBBER, stream%file_ID) )
